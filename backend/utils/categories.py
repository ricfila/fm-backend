...
async def collapseCategories(
    ...
    for c in categories:
        actual_parent_id = getattr(c, parent_field)

        # Collapse only if exists a parent category and if the parent category is the selected one
        if actual_parent_id is not None and (not parent_id or actual_parent_id == parent_id):

            # Check if parent category is already choosen for the order
            if not actual_parent_id in [k.id for k in categories + new_categories]:
                new_c = await Category.get_or_none(id=actual_parent_id, using_db=connection)
...
async def collapseOrphanCategories(
    categories: list[Category],
    changes: dict,
    connection: BaseDBAsyncClient
) -> tuple[list[Category], dict]:
    ...
    for c in categories:
        ...
        if parent_id is not None:
            if not any(k.id == parent_id for k in categories + new_categories):
                ...
                # Category is orphan: keep only parent
                new_categories.append(new_c)
                changes[c.id] = parent_id
            
            else:
                # Category is not orphan: keep it
                new_categories.append(c)
...
def collapseChangesChains(changes: dict) -> dict:
    resolved_changes = {}
